...
$dtc2=date("Y-m-31",strtotime('-1 months'));
...
//previa 3
$ades3=0;
$dtc=date("Y-m-01",strtotime('-2 months'));
$dtc2=date("Y-m-31",strtotime('-2 months'));
...
$dtc2=date("Y-m-31",strtotime('-3 months'));
...
//previa 5
$ades5=0;
$dtc=date("Y-m-01",strtotime('-4 months'));
$dtc2=date("Y-m-31",strtotime('-4 months'));
...
//previa 6
$ades6=0;
$dtc=date("Y-m-01",strtotime('-5 months'));
$dtc2=date("Y-m-31",strtotime('-5 months'));
 ...
 //previa 13
$ades13=0;
$dtc=date("Y-m-01",strtotime('-12 months'));
$dtc2=date("Y-m-31",strtotime('-12 months'));
...
if($tot!=0.0){
if($escrever["tilan"]=="MATRICULA") $timat4++;
if($escrever["tilan"]=="RETORNO") $tiret4++;
}

if($tot!=0.0) $npag4=$npag4+1;
...
$tot4=$totdin+$totcar+$totche+$totcarpi+$totcartr;
...
if($tot!=0.0){
if($escrever["tilan"]=="MATRICULA") $timat5++;
if($escrever["tilan"]=="RETORNO") $tiret5++;
}

if($tot!=0.0) $npag5=$npag5+1;

}/*Fim do while*/


$tot5=$totdin+$totcar+$totche+$totcarpi+$totcartr;
...
if($tot!=0.0){
if($escrever["tilan"]=="MATRICULA") $timat6++;
if($escrever["tilan"]=="RETORNO") $tiret6++;
}


if($tot!=0.0) $npag6=$npag6+1;

}/*Fim do while*/


$tot6=$totdin+$totcar+$totche+$totcarpi+$totcartr;
...
if($tot!=0.0){
if($escrever["tilan"]=="MATRICULA") $timat7++;
if($escrever["tilan"]=="RETORNO") $tiret7++;
}


if($tot!=0.0) $npag7=$npag7+1;

}/*Fim do while*/


$tot7=$totdin+$totcar+$totche+$totcarpi+$totcartr;
...
if($tot!=0.0){
if($escrever["tilan"]=="MATRICULA") $timat8++;
if($escrever["tilan"]=="RETORNO") $tiret8++;
}


if($tot!=0.0) $npag8=$npag8+1;
...
$tot8=$totdin+$totcar+$totche+$totcarpi+$totcartr;
...
if($tot!=0.0){
if($escrever["tilan"]=="MATRICULA") $timat13++;
if($escrever["tilan"]=="RETORNO") $tiret13++;
}


if($tot!=0.0) $npag13=$npag13+1;

}/*Fim do while*/


$tot13=$totdin+$totcar+$totche+$totcarpi+$totcartr;
...
$totcu13=$totcu13+$escrever["valor"];
...
$fattat = number_format($tot2, 2, ',', '.');
$tmed = number_format($tmed, 2, ',', '.');


$conn->close();

$npagmes=array($npag13,$npag8,$npag7,$npag6,$npag5,$npag4,$npag);
$fattmes=array($tot13,$tot8,$tot7,$tot6,$tot5,$tot4,$tot2);
$cusmes=array($totcu13,$totcu6,$totcu5,$totcu4,$totcu3,$totcu2,$totcu);

$totcu = number_format($totcu, 2, ',', '.');
$permes=array(date("m/Y",strtotime('-12 months')),date("m/Y",strtotime('-5 months')),date("m/Y",strtotime('-4 months')),date("m/Y",strtotime('-3 months')),date("m/Y",strtotime('-2 months')),date("m/Y",strtotime('-1 months')),date("m/Y"));

$ar = array('fatt'=>$fattat,'fattmes'=>$fattmes,'cusmes'=>$cusmes,'npagmes'=>$npagmes,'permes'=>$permes,'alat' => $alat,'ades' => $ades,'tmed'=>$tmed,'npag'=>$npag,'pfat'=>$pfat,
...
$retmes=array($tiret13,$tiret8,$tiret7,$tiret6,$tiret5,$tiret4,$tiret);
$matmes=array($timat13,$timat8,$timat7,$timat6,$timat5,$timat4,$timat);
$aludes=array($ades13,$ades6,$ades5,$ades4,$ades3,$ades2,$ades);

for($i=0;$i<$numope;$i++){
$arrope[$i][0]=substr($arrope[$i][0],0,10);
}

$ar = array('nope'=>$numope,'res'=>$arrope,'retmes'=>$retmes,'matmes'=>$matmes,'permes'=>$permes,'aludes'=>$aludes);
